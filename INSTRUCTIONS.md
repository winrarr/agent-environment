...
- Write backlog items in terms of the goal, rationale, constraints, and verifiable acceptance criteria. Treat implementation details as context or hypotheses rather than instructions: the agent picking up the item should inspect the current state, explore independently, and change the implementation approach when new evidence warrants it while still meeting the stated goal and constraints.
...
- Repository-specific GitHub tokens are stored in `.env/`; consult its local `README.md` for repository-specific usage before authorized GitHub operations. Treat the directory and its contents as secrets: never display, log, commit, or include their values in project artifacts.
